...
UniRep is a recurrent neural network model trained on 25 million protein sequences,
and has shown utility in protein engineering.
Its original implementation, however, has rough spots in its implementation,
...
UniRep is a recurrent neural network, trained using self-supervision
to predict the next amino acid in a sequence.
...
The original model was implemented in TensorFlow 1.13,
and its original API only allowed for one sequence to be transformed at once.
...
harnessing vectorization to make it fast.
...
JAX was our choice, because it provides automatic differentiation machinery
on top of the highly idiomatic and widely-used NumPy API.
JAX uses a number of components shared with TensorFlow,
in particular the use of the XLA (Accelerated Linear Algebra) library
...
we can also use it to implement model fine-tuning through gradient descent.
...
In the process, we also generated tests.
...
With the test suite,
we are confident that our reimplementation
at least returns correctly-shaped tensors.

During the reimplementation,
we found that certain operations that we took for granted to be implemented one way
...
It took digging through the TensorFlow source code
...
but added tests to guarantee that they were robust.

As it turns out, one way that deep learning models can be fragile
...
it is the protein sequence that is.
Thus, having robustly tested functions that generate the input tensors
is crucial to having confidence that the model works correctly end-to-end.
...
Evotuning is an important task when using UniRep,
and we provide a convenient API through the `evotune()` function.
There, our use of Optuna to automatically fine-tune weights
using the protocol that the original authors describe
enables end-users to "set and forget" the model fitting protocol
